...
export default function AddTestCase() {
  ...
  const refreshTestCases = async () => {
    try {
      ...
      setError(`Failed to load test cases. Is the backend running on ${API_BASE_URL}?`);
    }
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setError(`Failed to load reference data. Is the backend running on ${API_BASE_URL}?`);
      }
    })();
    refreshTestCases();
  }, []);
      ...
      <div className="atc-card" style={{ marginBottom: '1rem' }}>
        ...
        <div className="atc-actions">
          <a
            ...
            href={`${API_BASE_URL}/api/test-cases/bulk-template`}
          >
