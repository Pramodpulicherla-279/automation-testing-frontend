...
const WS_URL = WS_TEST_STATUS_URL;
...
const LogConsole = ({ logs, statusMode = 'idle' }) => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const res  = await fetch(`${API_BASE_URL}/api-testing/logs`);
                ...
                <div className="dashboard-left-panel">
                    ...
                    <div className="dashboard-card">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '12px', marginBottom: '12px', borderBottom: '1px solid #E2E8F0' }}>
                            ...
                            {/* Killing Appium mid-run would take the live test down with it. */}
                            <button
                                onClick={toggleAppium}
                                disabled={isRunning}
                                style={{ padding: '5px 12px', borderRadius: '6px', border: appiumStatus === 'running' ? '1px solid #FECACA' : '1px solid #BFDBFE', backgroundColor: appiumStatus === 'running' ? '#FEF2F2' : '#EFF6FF', color: appiumStatus === 'running' ? '#DC2626' : '#2563EB', cursor: isRunning ? 'not-allowed' : 'pointer', opacity: isRunning ? 0.5 : 1, fontWeight: 700, fontSize: '0.72rem', fontFamily: 'inherit', transition: 'all 0.15s' }}>
                    ...
                    {/* Network Config */}
                    <NetworkConfigPanel setNetworkConfig={setNetworkConfig} isRunning={isRunning} />
