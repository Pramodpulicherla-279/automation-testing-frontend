            ...
            <nav style={{ flex: 1, padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {NAV_ITEMS.map(({ label, to, icon }) => (
                    <NavLink
                        key={to}
                        to={to}
                        // Navigating away mid-run unmounts the live test screen.
                        onClick={e => { if (isRunning) e.preventDefault(); }}
                        aria-disabled={isRunning || undefined}
                        tabIndex={isRunning ? -1 : undefined}
                        title={isRunning ? 'Navigation is locked while a test run is in progress' : (collapsed ? label : undefined)}
                        style={({ isActive }) => ({
                            display: 'flex',
                            alignItems: 'center',
                            gap: '10px',
                            ...
                            ...(isRunning ? { opacity: 0.4, cursor: 'not-allowed', pointerEvents: 'none' } : {}),
                        })}
                        onMouseEnter={e => {
